...
import {imageUtility} from "../data/ImageUtilityClass";
...
const ConferenceBanner = () => {
    const ConferenceData = ['ﺍﻻﻳﺎﻡ', 'ﺍﻟﺴﺎﻋﺎﺕ', 'ﺍﻟﺪﻗﺎﺋﻖ', 'ﺍﻟﺜﻮﺍﻧﻲ']
    const [imageSrc, setImageSrc] = useState(personImage);

    useEffect(() => {
        const handleResize = () => {
            if (window.innerWidth <= 985) {
                setImageSrc(imageUtility.mobileViewPersonImage); // Mobile image
            } else {
                setImageSrc(imageUtility.personImage); // Default image
            }
        };
    ...
    return (
        <Base>
            <div style={{marginLeft: "5%", marginRight: "5%"}}>
                <Row className="conference-row" style={{position: "relative"}}>
                    ...
                    <Col xxl={7} xl={7} lg={7}>
                        <div className="topBoxes d-none d-lg-flex">
                            {ConferenceData.map((text, index) => (
                                <Card
                                    className={`conference-card ${index === 0 ? 'cardLeft' : ''} ${index === 3 ? 'cardRight' : ''}`}
                                    key={index}>
                                    <CardText className="conference-card-heading">00</CardText>
                                    <CardText className="conference-card-text">{text}</CardText>
                                </Card>
                            ))}
                        </div>

                        <div className="bottomBox">
                            <p className="conference-paragraph">ﺑﺮﻋﺎﻳﺔ ﻣﻌﺎﻟﻲ ﻭﺯﻳﺮ ﺍﻟﻌﺪﻝ ﺭﺋﻴﺲ ﻣﺠﻠﺲ
                                ﺇﺩﺍﺭﺓ ﺍﻟﻬﻴﺌــــﺔ ﺍﻟﺴﻌـــﻮﺩﻳﺔ ﻟﻠﻤﺤﺎﻣﻴـــﻦ
                            </p>
                            <p className="conference-heading">ﺩ. ﻭﻟﻴﺪ ﺑﻦ ﻣﺤﻤﺪ ﺍﻟﺼﻤﻌﺎﻧﻲ</p>
                            <p className="aboutConference">ﻋـﻦ ﺍﻟﻤﺆﺗﻤـﺮ</p>
                        </div>
                    </Col>
                </Row>
                        ...
                        <Card className="customBox">
                            ...
                            <Row style={{width: "85%"}}>
                                <Col xs={3} md={4} lg={4} className="mb-2 custom_slider_img_p">
                                    <img className="custom_slider_img" src={data[currentIndex].img} alt=""
                                         width={300}/>
                                </Col>
                                <Col xs={9} md={8} lg={8}
