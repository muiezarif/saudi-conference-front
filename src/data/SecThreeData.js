const cardData = [
    {number: '+10', name: 'شركاء',},
    {number: '+2000', name: 'زائز'},
    {number: '8', name: 'جلسات حوارية'},
    {number: '18', name: 'ورشة عمل'},
    {number: '+30', name: 'متحدث وخبير'},
];
const cardData2 = [
    // {name: 'ﻓﺌــــــــــــــــــــﺔ', name1: 'الإعلاميين', cur: 'ﺭﻳــــــــــﺎﻝ', number: '0'},
    {name: ' الفئــــــــــــــــــــﺔ', name1: 'طالب', cur: 'ﺭﻳــــــــــﺎﻝ', number: '400'},
    {name: 'الفئــــــــــــــــــــﺔ', name1: 'مشارك', cur: 'ﺭﻳــــــــــﺎﻝ', number: '1200'},
    {name: 'ﻓﺌــــــــــــــــــــﺔ', name1: 'VIP', cur: 'ﺭﻳــــــــــﺎﻝ', number: '3000'},
];
const cardData3 = [
    {name: 'ﺍﻟﻔﻀــــــــــﻲ', cur: 'فرص', number: '6'},
    {name: ' ﺍﻟﺬﻫﺒـــــــــﻲ', cur: 'فرص', number: '5'},
    {name: 'ﺍﻟﺒﻼﺗﻴﻨـــــــﻲ', cur: 'فرص', number: '4'},
    {name: 'الاستراتيجية', cur: '', number: 'رعاية حصرية'},
];
const workshops2 = [
    {title: 'عنوان الجلسة 1', date: '2024/12/23', time: '9 - 10 صباحًا'},
    {title: 'عنوان الجلسة 2', date: '2024/12/23', time: '11 - 12 صباحًا'},
    {title: 'عنوان الجلسة 3', date: '2024/12/23', time: '1 - 2 صباحًا'},
];

module.exports = {cardData, cardData2, cardData3, workshops2}